...
logger_filename = 'logs/random_rate_decreased/0.015.log'
save_path = 'model_weights/random_rate_decreased/0.015.pth'
decrease_rate = 0.015
...
class BiModel(nn.Module):
    def __init__(self): #此处point，counter都为正样本
        ...
        self.model1 = BertModel.from_pretrained('bert-base-uncased',mirror=mirror)
        self.tokenizer1 = BertTokenizer.from_pretrained('bert-base-uncased',mirror=mirror)
        self.linear1 = nn.Linear(768,128) #sim header
        self.linear2 = nn.Linear(2304,2)
    ...
    def classify_pair(self,emb1_1,emb1_2,emb2_1,emb2_2):
        ...
        x = torch.cat([x2_1, x2_2, torch.abs(x2_diff)], dim=1)  # 差别 以及差别的绝对值
        x = self.linear2(x)
        return x
...
for i in range(epochs):
    ...
    negative_index = searcher.search(([i for i in range(point_sim_embeddings_.shape[0])], point_sim_embeddings_),
                                     1.0 - i * decrease_rate if 1.0 - i * decrease_rate > 0 else 0 )
